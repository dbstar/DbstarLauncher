...
public class GDDataAccessor {
	...
	public String getDatabaseFile() {
		return new String(mStorageDir + "/" + DVBDATABASE_File);
	}
	...
	public String getMediaFile(ContentData content) {
		...
		file = mStorageDir + mPushPath + content.Mainfile;
	...
	public String getThumbnailFile(ContentData content) {
		String file = "";
		List<String> files = content.PosterFiles;
		if (files.size() > 0) {
			for (int i = 0; i < files.size(); i++) {
				file = new String(mStorageDir + mPushPath + files.get(i));
